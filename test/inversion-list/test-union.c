...
int main(void) {
  inversion_list_init();
  {
    ...
    InversionList *set3 =
        inversion_list_create(30, sizeof c / sizeof *c, c);

    set1 = inversion_list_union(set, set2, set3, null);
    assert(strcmp(inversion_list_to_string(set1),
                  "[1, 2, 3, 5, 7, 9, 10, 12, 23]") == 0);
    inversion_list_destroy(set1);

    set1 = inversion_list_union(set, set2, null);
    assert(strcmp(inversion_list_to_string(set1),
                  "[1, 2, 3, 5, 7, 9, 10]") == 0);
